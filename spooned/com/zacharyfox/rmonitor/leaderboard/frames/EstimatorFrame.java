...
public class EstimatorFrame extends JFrame {
  ...
  private EstimatorFrame(@NotNull MainFrame mainFrame) {
    if (mainFrame == null)
      throw new IllegalArgumentException("[Spoon inserted check] null passed as parameter");
    this.mainFrame = mainFrame;
    getContentPane()
        .setLayout(new net.miginfocom.swing.MigLayout("", "[grow][]", "[100.00,grow][][][][][]"));
    ...
    estimator.addPropertyChangeListener(
    ...
    topThree.setText(EstimatorFrame.this.getTopThreeText());
    EstimatorFrame.this.mainFrame.setEstimator(estimator);
  }
  ...
  private String getTopThreeText() {
    ...
    for (i = 1; i <= 3; i++) {
      ...
      if (comp != null) {
        text =
            (((text + (comp.getNumber())) + " ") + (RaceClass.getClassName(comp.getClassId())))
                + "<br />";
      }
    }
    text = text + "</body></html>";
  ...
  public static EstimatorFrame getInstance(@NotNull MainFrame mainFrame) {
    if (mainFrame == null)
      throw new IllegalArgumentException("[Spoon inserted check] null passed as parameter");
    if ((EstimatorFrame.instance) == null) {
      EstimatorFrame.instance = new EstimatorFrame(mainFrame);
    }
    return EstimatorFrame.instance;
  }
}
