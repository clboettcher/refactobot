...
@SuppressWarnings(value = "Since15")
public class MainFrame extends JFrame implements ActionListener {
  ...
  public MainFrame() {
    ...
    MainFrame.this.setBounds(100, 100, 870, 430);
    MainFrame.this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
    MainFrame
        .this
        .getContentPane()
        .setLayout(
            new net.miginfocom.swing.MigLayout("", "[grow][grow]", "[][10:10:10][][][grow]"));
    titleBar = new JPanel();
    MainFrame.this.getContentPane().add(titleBar, "cell 0 0 2 1,grow");
    titleBar.setLayout(new GridLayout(1, 0, 0, 0));
    runName = new JLabel("-");
    runName.setFont(
        new Font(systemLabelFont.getName(), Font.BOLD, ((systemLabelFont.getSize()) + 3)));
    ...
    timeBar.setLayout(
        new net.miginfocom.swing.MigLayout("", "[50:50:50][grow][grow][grow]", "[][]"));
    ...
    menuBar = new LeaderBoardMenuBar(MainFrame.this);
  ...
  public void actionPerformed(@NotNull ActionEvent e) {
    ...
    if (e.getActionCommand().equals("Connect")) {
      race = new Race();
      race.addPropertyChangeListener(
      ...
      worker = new Worker(ConnectFrame.getInstance(MainFrame.this), race);
      if ((recorder) != null) {
  ...
  public void removeEstimator() {
    MainFrame.this.estimator = null;
    worker.removeEstimator();
  }

  public void removeRecorder() {
    MainFrame.this.recorder = null;
    worker.removeRecorder();
  }

  public void setEstimator(@NotNull Estimator estimator) {
    if (estimator == null)
      throw new IllegalArgumentException("[Spoon inserted check] null passed as parameter");
    if ((MainFrame.this.estimator) == null) {
      MainFrame.this.estimator = estimator;
      worker.setEstimator(estimator);
    }
  }

  public void setRecorder(@NotNull Recorder recorder) {
    if (recorder == null)
      throw new IllegalArgumentException("[Spoon inserted check] null passed as parameter");
    if ((MainFrame.this.recorder) == null) {
      MainFrame.this.recorder = recorder;
      worker.setRecorder(recorder);
    }
  }

  private void setFlagColor(@NotNull String status) {
    if (status == null)
      throw new IllegalArgumentException("[Spoon inserted check] null passed as parameter");
  ...
  private void updateDisplay(@NotNull PropertyChangeEvent evt) {
    if (evt == null)
      throw new IllegalArgumentException("[Spoon inserted check] null passed as parameter");
